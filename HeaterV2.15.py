# Shed Temperature & Humidity Monitor v2.15 – FINAL with Heater Effectiveness Warning + Clean Shutdown
# • Countdown only on manual activation
# • Auto runtime now shows days if >24h
# • Button text shows correct action
# • Warn if heater runs >1h with <1°C temperature increase
# • Fixed TclError on window close (graceful thread stop)
# • Hold last good temp (with jump check) and humidity
...
HIGH_TEMP_THRESHOLD = 13.0
HUMIDITY_THRESHOLD = 75.0
MAX_MANUAL_RUNTIME = 40 * 60
HEATER_COOLDOWN = 10 * 60
GRAPH_HOURS = 36
FUTURE_HOURS = 1
MIN_TEMP_RISE = 1.0          # Minimum °C increase expected after 1 hour
WARNING_AFTER_HOURS = 1.0    # When to start checking
VERSION = "2.15"
...
class ShedMonitorApp:
    def __init__(self, root):
        ...
        # For "heater not heating" detection
        self.start_temp = None
        self.no_heat_warning_active = False

        # Hold last good values
        self.last_good_temp = None
        self.last_good_hum = None
        self.MAX_TEMP_JUMP = 3.0          # °C – reject bigger jumps for temperature only
        ...
        logging.info(f"=== Shed Monitor v{VERSION} STARTED ===")
    ...
    def toggle_heater(self):
        if not self.heater_on:
            ...
            self.start_temp = self.temps[-1] if self.temps else None
            self.no_heat_warning_active = False
    ...
    def sensor_loop(self):
        while self.running:
            try:
                h, t = Adafruit_DHT.read_retry(DHT_SENSOR, DHT_PIN, retries=6, delay_seconds=1.5)
                now = time.time()

                # ---------- Temperature (with jump check) ----------
                plot_temp = t
                if t is not None and 0.0 <= t <= 50.0:
                    if self.last_good_temp is not None:
                        if abs(t - self.last_good_temp) > self.MAX_TEMP_JUMP:
                            # Jump too large → keep previous value for the graph
                            plot_temp = self.last_good_temp
                            logging.warning(
                                f"Temp jump rejected for plot: {self.last_good_temp:.1f} → {t:.1f}°C"
                            )
                        else:
                            self.last_good_temp = t
                    else:
                        self.last_good_temp = t
                else:
                    # None or out-of-range → hold last good temperature
                    if self.last_good_temp is not None:
                        plot_temp = self.last_good_temp
                        logging.warning(f"Invalid temp ({t}) – holding last good value")
                    # else leave as None until we get the first good reading

                # ---------- Humidity (hold last good, no jump check) ----------
                plot_hum = h
                if h is not None and 0.0 <= h <= 100.0:
                    self.last_good_hum = h
                else:
                    # None or out-of-range → use last good humidity
                    if self.last_good_hum is not None:
                        plot_hum = self.last_good_hum
                        logging.warning(f"Invalid humidity ({h}) – holding last good value")
                    # else leave as None until first good reading

                # Only update the GUI once we have at least a temperature
                if plot_temp is not None:
                    self.root.after(0, self.update_gui, plot_temp, plot_hum, now)
                else:
                    self.root.after(0, lambda: self.msg_lbl.config(
                        text="Waiting for first valid sensor reading…", fg="yellow"))

            except Exception as e:
                logging.error(f"Sensor loop exception: {e}")
                # On hard error also hold last known good values
                if self.last_good_temp is not None:
                    self.root.after(0, self.update_gui,
                                    self.last_good_temp,
                                    self.last_good_hum if self.last_good_hum is not None else 0,
                                    time.time())
    ...
    def update_gui(self, temp, hum, now):
        ...
        # Heater effectiveness warning
        if self.heater_on and self.start_temp is not None:
            elapsed_hours = (now - self.heater_on_time) / 3600.0
            temp_increase = temp - self.start_temp
            
            if elapsed_hours >= WARNING_AFTER_HOURS:
                if temp_increase < MIN_TEMP_RISE:
                    if not self.no_heat_warning_active:
                        logging.warning(f"Heater running {elapsed_hours:.1f}h but temp change only {temp_increase:+.1f}°C")
                        self.no_heat_warning_active = True
                    warning_msgs.append(
                        f"WARNING: Heater ON for >{WARNING_AFTER_HOURS}h but temp only changed by {temp_increase:+.1f}°C!\n"
                        "→ Check heater / element / fan / power / airflow / thermostat"
                    )
                else:
                    if self.no_heat_warning_active:
                        logging.info(f"Heater effectiveness warning cleared (temp rose {temp_increase:.1f}°C)")
                    self.no_heat_warning_active = False

        if hum > HUMIDITY_THRESHOLD:
            msgs.append(f"High humidity {hum:.0f}%")

        all_msgs = msgs + warning_msgs
        if all_msgs:
            text = "\n".join(all_msgs)
            is_critical = any("WARNING" in m for m in warning_msgs)
            fg_color = "orange" if is_critical else "red"
            self.msg_lbl.config(text=text, fg=fg_color)
        else:
            self.msg_lbl.config(text="Status: Normal", fg="white")

    def shutdown(self):
        self.running = False              # Stop sensor loop
        if self.heater_on:
            self.send("off")
...
if __name__ == "__main__":
    ...
    app = ShedMonitorApp(tk.Toplevel())  # Use a new toplevel as main window
    app.root.mainloop()