# Shed Temperature & Humidity Monitor v2.12 – FINAL
# • Clean 6-hour labels + date at midnight
# • Graph extends 1 hour into the future
# • Rich blue background + bright readable text
...
class ShedMonitorApp:
    def __init__(self, root):
        ...
        self.btn = tk.Button(frame, text="Toggle Heater", command=self.toggle_heater,
        ...
        logging.info("=== Shed Monitor v2.12 STARTED ===")

    def send(self, state): os.system(f"sudo ~/rpitx/sendiq -s 250000 -f 433.9000e6 -t u8 -i ~/rpitx/heat{state}.iq")
    ...
    def update_runtime(self):
        if self.heater_on and self.heater_on_time:
            ...
            left = max(0, 40 - m) if self.manually_activated else "-"
            self.runtime_lbl.config(text=f"Runtime: {m}m {s}s | {left}m left")
        else:
    ...
    def shutdown(self):
        if self.heater_on: self.send("off")
