...
class ShedMonitorApp:
    ...
    def update_graph(self):
        """Update the temperature graph for the last 36 hours."""
        self.ax.clear()
        if self.temp_data:
            ...
            self.ax.plot(times, temps, color='white', linewidth=2)
            self.ax.set_xlabel("Time")
            self.ax.set_ylabel("Temperature (°C)")
            self.ax.set_title("Temperature Over Last 36 Hours")
            self.ax.grid(True)
            # Format x-axis to show time nicely
            self.fig.autofmt_xdate()
            # Set y-axis limits with some padding
            min_temp = min(temps) - 1 if temps else 0
            max_temp = max(temps) + 1 if temps else 20
            self.ax.set_ylim(min_temp, max_temp)
            # Set x-axis to show last 36 hours
            current_time = datetime.now()
            self.ax.set_xlim(current_time - timedelta(hours=36), current_time)
