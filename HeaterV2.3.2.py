...
class ShedMonitorApp:
    ...
    def update_gui(self, humidity, temperature, error=None):
        ...
        self.ax.tick_params(colors='#E5E7EB', labelsize=10)
        ...
        if self.temp_data:
            ...
            self.ax.axhline(y=LOW_TEMP_THRESHOLD, color='#F87171', linestyle='--', alpha=0.8)
            self.ax.axhline(y=HIGH_TEMP_THRESHOLD, color='#FBBF24', linestyle='--', alpha=0.8)

        self.ax.set_xlim(start, end)

        # ← THIS IS THE FIX – much cleaner spacing
        self.ax.xaxis.set_major_locator(HourLocator(interval=6))      # label every 6 hours
        self.ax.xaxis.set_minor_locator(HourLocator(interval=1))      # tiny tick every hour (no label)
        self.ax.xaxis.set_major_formatter(DateFormatter('%H:%M'))     # 14:30 format

        # Optional: show date only at midnight (super clean)
        def format_with_date(x, pos):
            dt = mdates.num2date(x)
            if dt.hour == 0:
                return dt.strftime('%d %b\n%H:%M')
            return dt.strftime('%H:%M')
        from matplotlib.dates import num2date, AutoDateLocator
        self.ax.xaxis.set_major_formatter(plt.FuncFormatter(format_with_date))

        self.fig.autofmt_xdate(bottom=0.2, rotation=0, ha='center')
